...
var (
	helpMsg = `ClipSync - Universal Clipboard
	With ClipSync, you can copy from one device and paste on another.
	
	Usage: clipsync [--secure/-s] [--debug/-d] [ <address> | --help/-h ]
	Examples:
	   clipsync                                   # start a new clipboard
	   clipsync -p 6666                           # start a new clipboard on a set port number
	   clipsync 192.168.86.24:53701               # join the clipboard at 192.168.86.24:53701
	   clipsync -d                                # start a new clipboard with debug output
	   clipsync -d --secure 192.168.86.24:53701   # join the clipboard with debug output and enable encryption
	Running just ` + "`clipsync`" + ` will start a new clipboard.
