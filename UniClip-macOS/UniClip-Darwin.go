...
func showConnectDialog() {
	a := app.New()
	w := a.NewWindow("Connect to UniClip Server")

	ipEntry := widget.NewEntry()
	ipEntry.SetPlaceHolder("Enter IP Address")

	content:=container.NewVBox(
		widget.NewLabel("Connect to UniClip Server"),
		ipEntry,
...
func makeServer() {
	...
	serverAddress := getOutboundIP().String() + ":" + port
	fmt.Println("Run", "`uniclip", serverAddress, "to join this clipboard")
	fmt.Println()

	sendMacOSNotifictation("UniClip Server", "Running at:"+serverAddress)
	for {
		c, err := l.Accept()
		if err != nil {
			handleError(err)
...
func sendMacOSNotifictation(title, message string) {
	script := fmt.Sprintf(`display notification "%s" with title "%s"`, message, title)
	cmd := exec.Command("osascript", "-e", script)
	err := cmd.Run()
	if err != nil {
		log.Println("Failed to send macOS notification:", err)
	}
}
